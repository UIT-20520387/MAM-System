  ...
  if (
    ...
    return res
      ...
      .json({
        success: false,
        message:
          "Thiếu thông tin bắt buộc: apartment_id, type_id, apartment_number, area, price, status.",
      });
  }
  ...
  try {
    ...
    if (error) {
      ...
      if (error.code === "23505") {
        return res
          .status(409)
          .json({ success: false, message: "Mã căn hộ này đã tồn tại." });
      }
      ...
      return res
        .status(500)
        .json({ success: false, message: "Lỗi hệ thống khi thêm căn hộ." });
    }
  ...
  try {
    ...
    if (error) {
      ...
      return res
        ...
        .json({
          success: false,
          message: "Lỗi hệ thống khi tải danh sách căn hộ.",
        });
    }
  ...
  try {
    ...
    if (error) {
      if (error.code === "PGRST116") {
        return res
          .status(404)
          .json({ success: false, message: "Không tìm thấy căn hộ này." });
      }
      ...
      return res
        ...
        .json({
          success: false,
          message: "Lỗi hệ thống khi tải chi tiết căn hộ.",
        });
    }
  ...
  try {
    ...
    if (data.length === 0) {
      return res
        ...
        .json({
          success: false,
          message: "Không tìm thấy căn hộ để cập nhật trạng thái.",
        });
    }
